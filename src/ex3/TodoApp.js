...
function TodoApp() {
  const [todos, setTodos] = useState(() => {
    ...
    if (tarefasGuardadas) {
      return JSON.parse(tarefasGuardadas);
    } else {
      return []; 
    }
  });
  ...
  useEffect(() => {
    document.title = `Tens ${todos.length} tarefas pendentes!`; 
    localStorage.setItem('minhasTarefas', JSON.stringify(todos));
  }, [todos]);
